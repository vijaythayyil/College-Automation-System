...
<div class="subsec">
    <div class="bar">
        <li>Placement Officers</li>
        <input type="submit" value="Add New" class="btn btn-info btn-sm mt-3 mr-4 pull-right" onclick="window.location.href='placement.php?addnew'">
    </div>
    <?php 
    ...
    <table class="table table-hover" style="float: left;">
        ...
        <tbody>
        <?php
           $sql = "SELECT * FROM department d inner join placement p on p.department_Id=d.department_Id";
        ...
        ?>
          ...
          <td><?php echo $row["id"]; ?></td>
            ...
            <td>
            <input type="submit" value="Delete" class="btn btn-danger btn-sm mr-2 pull-right" onclick="window.location.href='placement.php?delete=<?php echo $row['id']; ?>'">
            <input type="submit" value="Edit" class="btn btn-info btn-sm  mr-2 pull-right" onclick="window.location.href='placement.php?edit=<?php echo $row['id']; ?>'">
            </td>
          </tr>
        <?php
            }
        ...
        <div class="container" style="padding-top:15px; float:left;">
         <form action="" method="post" enctype="multipart/form-data">
           <div class="form-group" style="width:35%;">
             ...
             <input type="button" value="Cancel" class="btn btn-danger mt-3 ml-3 pull-left" onclick="window.location.href='placement.php?fetch'">
           </div>
         </form>
        </div>
   <?php 
       ...
       if(isset($_GET['edit']))
      {
        ...
        $sql="SELECT * FROM department d inner join placement p on p.department_Id=d.department_Id where p.id='$dptid'";
...
if(isset($_FILES['files']))
{
  ...
  while(($filesop = fgetcsv($handle, 1000, ",")) !== false)
  {
    ...
    $department_Id = $filesop[4];
      ...
      $sql = "INSERT INTO placement(id,name,username,password,department_Id) VALUES ('$id','$name','$username','$password','$department_Id')";
  ...
  {
    ?>  
    <script> alert('Placement Officers added to Database');window.location.href='placement.php?fetch';</script>
    <?php
  } 
  else
  {
    ?>  
    <script> alert('Sorry! Unable to import');window.location.href='placement.php?addnew';</script>
    <?php
  }
}
...
if(isset($_POST['update']))
{
    ...
    $sql="UPDATE `placement` SET `name`='$nama',`department_Id`='$department',`username`='$username',`password`='$password' WHERE `Id`='$id'";
    if (mysqli_query($conn, $sql)) 
    {
     ?>
     <script> alert('Placement Updated');window.location.href='placement.php?fetch';</script>
     <?php
    }
...
if(isset($_GET['delete']))
{
    ...
    {
     ?>
     <script> alert('Placement Removed');window.location.href='placement.php?fetch';</script>
     <?php
    }
