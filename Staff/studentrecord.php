...
<div class="subsec">
    <div class="bar">
        <li>Student Records</li>     <!-- added by vijay-->
        <form action="" method="post">
        <input type="button" value="Search" class="btn btn-success m-2 pull-right"  onclick="window.location.href='search.php?fetch'"> 
       <!-- <input type="" name="yadm" placeholder="Enter Student ID" class="form-control m-2" style="width:25%; float:right;"/>  -->
        </form>
    </div>
    ...
    <table class="table table-hover" style="float: left;">
        ...
        <?php
          ...
          //i added below these to add marklist functionality
            //upto here marklist fn....down below is sorting for record of student

          $sql = "SELECT * FROM department d inner join student s on s.department_Id=d.department_Id inner join mark m on m.student_Id=s.student_Id  where s.department_Id='$d_id' group by m.semester,m.student_id order by year_of_admission desc";
        ...
        ?>
          <tr>
            <td><?php echo $row["student_id"];$std=$row["student_id"];?></td>
            <td><?php echo $row["rollno"]; ?></td>
            <td><?php echo $row["name"]; ?></td>
            <td><?php echo $row["department"]; ?></td>
            <td><?php echo $row["semester"]; ?></td>
            <td><?php echo $row["year_of_admission"]; ?></td>
      <!-- current semester is deleted-->
            <td><?php
            $sql3="SELECT AVG(sgpa)as cgpa FROM mark where student_Id='$std'"; 
            $result1= mysqli_query($conn,$sql3);
            $row1 = mysqli_fetch_assoc($result1); 
            $cgpa = $row1['cgpa']; 
            echo round($cgpa,2); ?></td>
...
if(isset($_FILES['files']))
{
  ...
  while(($filesop = fgetcsv($handle, 1000, ",")) !== false)
  {
      ...
      $sql = "insert into student(student_id,rollno,name,department_Id,year_of_admission,username,password) values('$student_id','$rollno','$name','$department_Id','$year_of_admission','$username','$password')";
